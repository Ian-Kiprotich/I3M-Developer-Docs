...
Let's start by making a new project using the special tiny tool - `i3m-template` - it allows you to generate all boilerplate
...
```shell
i3m-template init --name platformer --style 2d
```
...
the `i3m-template` generate quite some code for you. There are tiny comments about which place is for what. For more info
